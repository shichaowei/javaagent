...
public class MybatisTransformer implements ClassFileTransformer {
	...
	private static Logger logger = Logger.getLogger(MybatisTransformer.class);
...
//		methodList.add("org.apache.ibatis.executor.CachingExecutor.update");
	 ...
	 * businessJdbcUrl=jdbc:mysql://10.200.141.58:3306/fengdai_riskcontrol?useUnicode=true&characterEncoding=utf-8##
	 * businessJdbcName=fdtest##businessJdbcPassword=Mysqltest@123098##httpserver=192.168.16.18:8090"
	 *
	 * @param agentArgs
	 */
	public MybatisTransformer(String agentArgs) {
		...
		for (String element : args) {
			...
			switch (key) {
			case "businessJdbcUrl":
				businessJdbcUrl=value;
				break;
			case "businessJdbcName":
				businessJdbcName=value;
				break;
			case "businessJdbcPassword":
				businessJdbcPassword=value;
				break;
			case "httpserver":
				httpserver=value;
				break;
	...
	protected static String[] getMethodParamNames(CtMethod cm) {
		...
		/*
		 * int pos = Modifier.isStatic(cm.getModifiers()) ? 0 : 1; for (int i = 0; i <
		 * paramNames.length; i++) { System.out.println(i); paramNames[i] =
		 * attr.variableName(i + pos); System.out.println("方法的入参是 :"+paramNames[i]); }
		 */
			...
			try {
				...
				for (String method : methodList) {
					if (method.startsWith(className)) {
						...
						// System.out.println("classname*** is"+ctclass.getName()+"****method
						// is:"+method);
						// 获取方法名
						...
						// 增加一个方法，对于一个class只修改一次 不重复修改
						if (CachingExecutorFlag
								&& ctclass.getName().equals("org.apache.ibatis.executor.CachingExecutor")) {
						...
						if (SimpleExecutorFlag
								&& ctclass.getName().equals("org.apache.ibatis.executor.SimpleExecutor")) {
							CtMethod m = CtNewMethod.make(
									"public  String getParameterValue(Object obj) {String value = null;if (obj instanceof String) {value = \"'\" + obj.toString() + \"'\";} else if (obj instanceof java.util.Date) {java.text.DateFormat formatter = java.text.DateFormat.getDateTimeInstance(java.text.DateFormat.DEFAULT, java.text.DateFormat.DEFAULT, java.util.Locale.CHINA);value = \"'\" + formatter.format(new java.util.Date()) + \"'\";} else {if (obj != null) {value = obj.toString();} else {value = \"\";}}return value;};",
									ctclass);
							ctclass.addMethod(m);
							CtMethod m1 = CtNewMethod.make(
									...
									ctclass);
							ctclass.addMethod(m1);
							SimpleExecutorFlag = false;
						}
						/*
						 * if(ExecutorFlag
						 * &&ctclass.getName().equals("org.apache.ibatis.executor.BaseExecutor")){
						 * CtMethod m=CtNewMethod.
						 * make("public  String getParameterValue(Object obj) {String value = null;if (obj instanceof String) {value = \"'\" + obj.toString() + \"'\";} else if (obj instanceof java.util.Date) {java.text.DateFormat formatter = java.text.DateFormat.getDateTimeInstance(java.text.DateFormat.DEFAULT, java.text.DateFormat.DEFAULT, java.util.Locale.CHINA);value = \"'\" + formatter.format(new java.util.Date()) + \"'\";} else {if (obj != null) {value = obj.toString();} else {value = \"\";}}return value;};"
						 * , ctclass); ctclass.addMethod(m); CtMethod m1=CtNewMethod.make(
						 * "public  String showSql(org.apache.ibatis.session.Configuration configuration, org.apache.ibatis.mapping.BoundSql boundSql)"
						 * + " {" + "Object parameterObject = boundSql.getParameterObject();" +
						 * "String sql = boundSql.getSql();" +
						 * "if (boundSql.getParameterMappings().size() > 0 && parameterObject != null) { "
...
//										+ "System.out.println(boundSql.getSql().toString());"
								...
								/*
								 * ctmethod.
								 * insertBefore("Object parameter = null;if ($2 !=null) { parameter = $2;}" +
								 * "org.apache.ibatis.mapping.BoundSql boundSql=$1.getBoundSql(parameter);" // +
								 * "org.apache.ibatis.session.Configuration configuration=$1.getConfiguration();"
								 * + "Object parameterObject=boundSql.getParameterObject();" +
						 ...
						 *
						 * String newMethodName = methodName + "$impl"; // 原来的方法改个名字
						 * ctmethod.setName(newMethodName); String type =
						 * ctmethod.getReturnType().getName();
						 *
						 * // 创建新的方法，复制原来的方法 ，名字为原来的名字 CtMethod newMethod = CtNewMethod.copy(ctmethod,
						 * methodName, ctclass, null);
						 * System.out.println(newMethod.getMethodInfo().toString()); // 构建新的方法体
						 * StringBuilder bodyStr = new StringBuilder(); bodyStr.append("{");
						 * bodyStr.append(prefix); // 调用原有代码，类似于method();($$)表示所有的参数 if
						 * (!"void".equals(type)) { bodyStr.append(type).append(" result = "); }
						 * bodyStr.append(newMethodName + "($$);\n");
						 *
						 * bodyStr.append("System.out.println(\"*****\"+result);");
						 * bodyStr.append(postfix); bodyStr.append(outputStr); if (!"void".equals(type))
						 * { bodyStr.append("return result;\n"); } bodyStr.append("}");
						 * System.out.println(bodyStr.toString()); // 替换新方法
						 * newMethod.setBody(bodyStr.toString()); // 增加新方法 ctclass.addMethod(newMethod);
						 */
					}
				}
